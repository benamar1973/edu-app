...
<style>
    ...
    * {{ font-family: 'Tajawal', sans-serif; }}
    ...
    header, footer, [data-testid="stHeader"], [data-testid="stFooter"] {{ display: none !important; }}
...
# --- كود الجافا سكريبت السحري لحذف الإشهار السفلي نهائياً ---
components.html("""
<script>
    function removeBadge() {
        // البحث عن زر السهم والقائمة الافتراضية وحذفهما
        const appParent = window.parent.document.querySelector('.stApp');
        if (appParent) {
            // استهداف عناصر شريط المنصة في الجذور وحذفها
            const badges = window.parent.document.querySelectorAll('[class^="viewerBadge_"], .stAppDeployDropdown, [data-testid="stStatusWidget"]');
            badges.forEach(b => b.remove());
            
            // إخفاء الحواف الإضافية التي تظهر بالأسفل بسبب الشريط
            const mainCont = window.parent.document.querySelector('.stAppViewMain');
            if (mainCont) { mainCont.style.paddingBottom = '0px'; }
        }
    }
    // تشغيل الحذف بشكل متكرر لضمان القضاء عليه بمجرد تحميل الصفحة
