...
<style>
    ...
    * {{
    ...
    .st-emotion-cache-1r6slb0, .st-emotion-cache-1w1c4f6 {{
        display: none !important;
    }}
</style>
""", unsafe_allow_html=True)

# إخفاء تام للأسفل (بديل عن popup)
components.html("""
<script>
...
# دوال فتح الرابط بدون popup blocker
def open_link(url):
    components.html(f"""
        <script>
...
with col1:
    if st.button("📖 درس نصي", use_container_width=True):
        if topic:
            ...
            open_link(url)
            st.success("✅ جاري فتح الدرس النصي...")
        else:
